...
int main(int argc,char *argv[])
{
  ...
  if (argc!=5)
  {
    ...
    printf("Example:/project/tools1/bin/procctl 120 /project/idc1/bin/obtcodetodb /project/idc/ini/stcode.ini \"192.168.226.128,root,1999,mysql,3306\" utf8 /log/idc/obtcodetodb.log\n\n");
  ...
  for(int i=0;i<vstcode.size();i++)
  {
    ...
    //执行插入的SQL语句,如果插入失败，原因1是已经存在，2是真的错了
    if(stmtins.execute()!=0)
    {
      //1、如果记录已存在，执行更新的SQL语句。
