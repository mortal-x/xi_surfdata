...
int main(int argc,char *argv[])
{
  ...
  if (argc!=5)
  {
    ...
    printf("Example:/project/tools1/bin/procctl 10 /project/idc1/bin/obtmindtodb /idcdata/surfdata \"192.168.226.128,root,1999,mysql,3306\" utf8 /log/idc/obtmindtodb.log\n\n");
...
bool _obtmindtodb(char *pathname,char *connstr,char *charset)
{
  ...
  while (true)
  {
    ...
    // 连接数据库。
    if (conn.m_state==0) //m_state数据库的连接状态，0未连接，1已连接
    {
    ...
    if (stmt.m_state==0) ////m_state数据库连接的绑定状态，0未绑定，1已绑定
    {
      stmt.connect(&conn);
    ...
    while (true)
    {
      ...
      if (strlen(tmp)>0) snprintf(stzhobtmind.t,10,"%d",(int)(atof(tmp)*10)); //如果有数据，转化为整型存到结构体里去
