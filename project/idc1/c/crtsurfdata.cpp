...
int main(int argc,char *argv[])
{
  ...
  if(argc!=5 && argc!=6){
    ...
    printf("Example:/project/idc1/bin/crtsurfdata /project/idc1/ini/stcode.ini /tmp/idc/surfdata /log/idc/crtsurfdata.log xml,json,csv 20210710123000\n");  //六个参数
  ...
  //关闭全部的信号和输入输出
  //设置信号，在shell状态下可用"kill + 进程号" 正常终止某些进程。
  ...
  PActive.AddPInfo(20,"crtsurfdata");  //把当前进程的心跳加入到共享内存数组中去。
  ...
  if(argc==5)
    ...
    STRCPY(strTime,sizeof(strTime),argv[5]);
...
bool CrtSurfFile(const char *outpath,const char *datafmt)
{
  ...
  if(strcmp(datafmt,"xml")==0) //xml文件。
  {
    File.Fprintf("<data>\n");
  }

  if(strcmp(datafmt,"json")==0) //json文件。
  {
    File.Fprintf("{\"data\":[\n");
  }
  ...
  { 
    ...
    if(strcmp(datafmt,"xml")==0) //xml文件
    {
      File.Fprintf("<obtid>%s</obtid><time>%s</time><temp>%.1f</temp><pressure>%.1f</pressure>"\
      " <RH>%d</RH>,<wind_dir>%d</wind_dir>,<wind_speed>%.1f</wind_speed><rainfall>%.1f</rainfall><vis>%.1f</vis><endl/>\n",\
      vsurfdata[i].obtid,vsurfdata[i].time,vsurfdata[i].temp/10.0,vsurfdata[i].pressure/10.0,\
      vsurfdata[i].RH,vsurfdata[i].wind_dir,vsurfdata[i].wind_speed/10.0,vsurfdata[i].rainfall/10.0,\
      vsurfdata[i].vis/10.0);
    }
    ...
    if(strcmp(datafmt,"json")==0) //json文件
    {
      File.Fprintf("{\"obtid\":\"%s\",\"time\":\"%s\",\"temp\":\"%.1f\",\"pressure\":\"%.1f\","\
      "\"RH\":\"%d\",\"wind_dir\":\"%d\",\"wind_speed\":\"%.1f\",\"rainfall\":\"%.1f\",\"vis\":\"%.1f\"}",\
      vsurfdata[i].obtid,vsurfdata[i].time,vsurfdata[i].temp/10.0,vsurfdata[i].pressure/10.0,\
      vsurfdata[i].RH,vsurfdata[i].wind_dir,vsurfdata[i].wind_speed/10.0,vsurfdata[i].rainfall/10.0,\
      vsurfdata[i].vis/10.0);
  ...
  if(strcmp(datafmt,"xml")==0) //如果是xml文件，写结束标签。
  {
    File.Fprintf("</data>\n");
  }
  
  if(strcmp(datafmt,"json")==0) //json文件结尾
  {
    File.Fprintf("]}\n");
  }  
  ...
  UTime(strFileName,strTime); //修改文件的时间属性。
