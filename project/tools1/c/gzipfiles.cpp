...
int main(int argc,char *argv[])
{
  ...
  if(argc!=4)
  {
    ...
    printf("Example: /project/tools1/bin/gzipfiles /tmp/idc/surfdata \"*.xml,*.json\" 0.01\n");
    printf("Example: /project/tools1/bin/procctl 300  /project/tools1/bin/gzipfiles /log/idc \".log.20*\" 0.02 \n");
    printf("Example: /project/tools1/bin/procctl 300  /project/tools1/bin/gzipfiles /tmp/idc/surfdata \"*.xml,*.json\" 0.01\n\n");

    printf("这是一个工具类，用于压缩历史数据文件或日志文件。\n");
  ...
  while(true)
  {
    ...
    if((strcmp(Dir.m_ModifyTime,strTimeOut)<0) && (MatchStr(Dir.m_FileName,"*.gz")==false)) //MatchStr为自定义匹配函数
    {
      //压缩文件，调用操作系统的gzip命令。
