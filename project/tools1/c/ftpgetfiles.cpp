...
struct st_arg
{
  ...
  char okfilename[301]; //已经下载成功文件清单。
...
vector<struct st_fileinfo> vlistfile2; //存放下载前列出服务器文件名的容器（nlist返回的结果）  
...
//如果ptype==1,把下载成功的文件记录追加到okfilename文件夹中
...
void _help()
{
  ...
  printf("<okfilename>/idcdata/ftplist/ftpgetfiles_surfdata.xml</okfilename>已经下载成功文件名清单,此参数只有当ptype=1时才有效\n");
...
bool _xmltoarg(char *strxmlbuffer)
{
  ...
  GetXMLBuffer(strxmlbuffer,"ptype",&starg.ptype);  //下载后服务器文件的处理方式：1-什么也不做；2-删除；3-备份
...
bool _ftpgetfiles()
{
  ...
  ******************************************************************/
  ...
  //把ftp.nilst()方法获取到的list文件加载到容器vlistfile2中。
